...
def context_and_entities_to_graph(context_result, corrected_entities):
    ...
    # --- Step 1: 添加 corrected_entities 的实体节点，并将非 Paper 节点连接到 Paper 节点 ---
    paper_nodes_in_corrected = []
    for entity_type, values in corrected_entities.items():
        for value in values:
            ...
            if entity_type == "Papers":
                paper_nodes_in_corrected.append(node_id)

    for entity_type, values in corrected_entities.items():
        if entity_type == "Papers":
            continue  
        for value in values:
            from_id = f"{entity_type}:{value}"
            for paper_id in paper_nodes_in_corrected:
                rel, direction = get_relationship_and_direction("Papers", entity_type)
                if direction == ("Papers", entity_type):
                    if not G.has_edge(paper_id, from_id):
                        G.add_edge(paper_id, from_id, label=rel)
                else:
                    if not G.has_edge(from_id, paper_id):
                        G.add_edge(from_id, paper_id, label=rel)
    ...
    for record in context_result:
        ...
        if paper_id:
            for key, val in record.items():
                ...
                rel, direction = get_relationship_and_direction("Papers", label)
                if direction == ("Papers", label):
                    if not G.has_edge(paper_id, node_id):
                        G.add_edge(paper_id, node_id, label=rel)
                else:
                    if not G.has_edge(node_id, paper_id):
                        G.add_edge(node_id, paper_id, label=rel)
                        
    # --- Step 3: 建立 corrected_entities 和 context_result 之间的连接（非 Paper 节点只连接到 Paper 节点） ---
    ...
    for ent_node in corrected_nodes:
        ...
        for ctx_node in context_nodes:
            ...
            if ent_label != "Papers" and ctx_label != "Papers":
                continue
...
RETURN p.title AS title, a.name AS author, p.year AS year, v.name AS venue
