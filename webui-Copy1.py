...
def save_chat_sessions():
    with open(CHAT_FILE, 'w', encoding='utf-8') as f:
        json.dump(st.session.state.chat_sessions, f, indent=4, ensure_ascii=False)
...
if "api_key" not in st.session_state:
    st.session_state.api_key = "sk-8eca9400641949349ba17730270b006b"
if "base_url" not in st.session_state:
    st.session_state.base_url = "https://dashscope.aliyuncs.com/compatible-mode/v1"
if "weaviate_url" not in st.session_state:
    st.session_state.weaviate_url = "https://aisrbu5qqwmbaknbgpcja.c0.asia-southeast1.gcp.weaviate.cloud"
if "weaviate_key" not in st.session_state:
    st.session_state.weaviate_key = "lKvgU6NgrDQ5WXfI818jyRok6ddMOfhBeQAL"
if "huggingface_key" not in st.session_state:
    st.session_state.huggingface_key = "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"
...
with st.sidebar:
    with st.expander("🔧 Setting", expanded=True):
        ...
        model_options = ['qwen-max-2025-01-25', 'qwen-max', 'qwen-max-latest', 'qwen-max-0428']  # 添加可选模型
...
question = st.text_area("Question：", height=150, placeholder="Please Enter Your Question:")
